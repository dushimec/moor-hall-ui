...
const AdminLayout: React.FC = () => {
  ...
  const navItems: NavItem[] = [
    ...
    { name: 'WhatsApp', href: '/admin/whatsapp', icon: ChatIcon },
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      {/* Sidebar */}
      <div
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#1a1a2e] text-white transform transition-transform duration-200 ease-in-out lg:translate-x-0 ${
          sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        <div className="flex h-16 items-center justify-between px-4 border-b border-gray-700">
          <Link to="/admin/dashboard" className="flex items-center space-x-2">
            <img src="/assets/logomoor.png" alt="Moor Hall" className="h-8 w-auto" />
            <span className="text-lg font-semibold">Moor Hall Admin</span>
          </Link>
          <button
            onClick={() => setSidebarOpen(false)}
        ...
        <div className="border-t border-gray-700 p-4">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-400 truncate">admin@moorhall.com</p>
            </div>
          </div>
        </div>
      </div>
